...
public class StudentQC extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		//查询是否存在
		List<StudentBean> list=new StudentDao().GetList("Student_Username='"+Student_Username+"' and Student_State='已手术'", "");
		if(list.size()<1)
		{
			out.print("<script language='javascript'>alert('住院号不存在，或病人未处于已手术状态！');history.back(-1);</script>");
			out.flush();out.close();return null;
		}
