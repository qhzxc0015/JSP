...
public class StudentRZSave extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		if(list.size()<1)
		{
			out.print("<script language='javascript'>alert('住院号不存在！');history.back(-1);</script>");
			out.flush();out.close();return null;
		}
		else
		{
			sbean=new StudentDao().GetAllFirstBean("Student_Username='"+Student_Username+"'");
			if(!(sbean.getStudent_State().equals("未手术")))
			{
				out.print("<script language='javascript'>alert('该病人处于"+sbean.getStudent_State()+"状态，禁止已手术操作！');history.back(-1);</script>");
				out.flush();out.close();return null;
			}
		}
		//修改
		...
		cnbean.setStudent_State("已手术");
		new StudentDao().Update(cnbean);
		    
		//跳转
		out.print("<script language='javascript'>alert('已手术操作成功！');window.location='StudentRZ.action';</script>");
		out.flush();out.close();return null;
